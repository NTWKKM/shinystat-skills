"""
Headless Logging Framework for Statistical Analysis in medstat.
"""
...
# Default configuration (can be overridden by config.py or environment)
...
class Logger:
    """Wrapper around standard logger with biostatistical convenience methods."""
    ...
    def log_data_summary(
        ...
        if DEFAULT_CONFIG.get("logging.log_data_operations"):
            numeric_count = sum(
                1 for t in dtypes.values() if "int" in t.lower() or "float" in t.lower()
            )
            object_count = sum(1 for t in dtypes.values() if "object" in t)
            self.info(
                f"📈 {df_name}: shape={shape}, numeric={numeric_count}, object={object_count}"
            )
    ...
    def log_analysis(
        ...
        if DEFAULT_CONFIG.get("logging.log_analysis_operations"):
            self.info(
                f"📈 {analysis_type}: outcome='{outcome}', predictors={n_vars}, n={n_samples}"
            )
