...
| **High** | $20\% - 40\%$ | Substantial missingness warranting comprehensive mechanism and sensitivity assessment. | Multiple imputation with elevated $m$ (scaled to FMI) and mandatory sensitivity analyses selected based on the target estimand and assessed missingness mechanism (e.g., pattern-mixture MNAR models when mechanism evidence suggests informative missingness, or complete-case comparison when MCAR is plausible). |
