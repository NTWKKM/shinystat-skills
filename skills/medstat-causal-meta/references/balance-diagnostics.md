...
| :--- | :--- | :--- | :--- |
| **$|\text{SMD}| < 0.10$** | **Well-balanced** | Negligible difference; groups are clinically comparable. | Proceed to outcome analysis. |
| **$0.10 \le |\text{SMD}| \le 0.20$** | **Moderate Imbalance** | Potential residual confounding across this covariate. | Re-estimate propensity score with interaction/polynomial terms, or include as covariate in outcome model (Doubly Robust). |
| **$|\text{SMD}| > 0.20$** | **Severe Imbalance** | Substantial difference; confounding bias likely. | Tighten caliper, trim non-overlapping support, or switch to propensity score weighting (IPTW). |
...
- Using a caliper of $0.2 \times \text{SD}(L)$ was shown by Austin to eliminate over 98% of the bias from measured linear covariates in simulation studies of matching estimators. It does not eliminate unmeasured confounding; residual unmeasured confounding still requires assessment.
